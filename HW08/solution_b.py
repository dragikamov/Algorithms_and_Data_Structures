...
class RedBlackTree:
  f=open("solution_b.txt",'w')
  def __init__(self):
    self.root = NilNode.instance()
    self.size = 0
    
  def __str__(self):
    self.f.write(("(root.size = %d)\n" % self.size)  + str(self.root))
    self.f.close()
    return ("(root.size = %d)\n" % self.size)  + str(self.root)

  def add(self, key):
    self.f.write("Adding " + str(key) + "\n")
    self.insert(Node(key))
    
  def insert(self, x):
    self.__insert_helper(x)
    self.f.write("Current color red\n")
    x.color = True
    while x != self.root and x.parent.color == True:
      if x.parent == x.parent.parent.left:
        ...
        if y and y.color == True:
          ...
          self.f.write(str(x.parent.key) + " changed to black\n")
          y.color = False
          self.f.write(str(y.key) + " changed to black\n")
          x.parent.parent.color = True
          self.f.write(str(x.parent.parent.key) + " changed to red\n")
          x = x.parent.parent
        else:
          if x == x.parent.right:
            x = x.parent
    ...
    self.f.write(str(self.root.key) + " changed to black\n")
  ...
  def __left_rotate(self, x):
    self.f.write("Rotating left: " + str(x.key) + "\n")
    if not x.right:
  ...
  def __right_rotate(self, x):
    self.f.write("Rotating right: " + str(x.key) + "\n")
    if not x.left:
      raise "x.left is nil!"
    y = x.left
    x.left = y.right
